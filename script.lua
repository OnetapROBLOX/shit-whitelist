...
local cfglocation = "Avidity.cfg/"      
makefolder("Avidity.cfg")      
...
function library:New(name)      
	...
	cursor.ZIndex = 1000 
	cursor.ImageColor3 = COL3RGB(238,26,33) 
...
makefolder("Avidity.lua")      

local allluas = {}  

for _,lua in pairs(listfiles("Avidity.lua")) do  
	local luaname = GSUB(lua, "Avidity.lua\\", "")  
...
luascripts:Element("Button", "load", {}, function()  
	loadstring(readfile("Avidity.lua\\"..values.luas["lua scripts"].lua.Scroll))()  
end)  
...
aimbot:Element("Toggle", "backtrack")  
    if values.rage.ragebot["backtrack"].Toggle then 
            args[1] = RageTarget 
...
coroutine.wrap(function()      
	while wait(1/16) do      
		LagTick = CLAMP(LagTick + 1, 0, values.aatab.fakelag.limit.Slider)      
		if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("UpperTorso") and values.aatab.fakelag.enabled.Toggle then    
			if LagTick == (values.aatab.fakelag.amount.Dropdown == "static" and values.aatab.fakelag.limit.Slider or RANDOM(1, values.aatab.fakelag.limit.Slider)) then      
				game:GetService("NetworkClient"):SetOutgoingKBPSLimit(9e9)      
				FakelagFolder:ClearAllChildren()      
				LagTick = 0      
				if values.aatab.fakelag["visualize lag"].Toggle then      
					for _,hitbox in pairs(LocalPlayer.Character:GetChildren()) do      
						if hitbox:IsA("BasePart") and hitbox.Name ~= "HumanoidRootPart" then      
							...
							part.Material = values.aatab.fakelag["visualize lag material"].Dropdown       
							part.Color = values.aatab.fakelag["visualize lag"].Color      
							part.Name = hitbox.Name      
							part.Transparency = values.aatab.fakelag["visualize lag transparency"].Slider/10        
				...
				if values.aatab.fakelag.enabled.Toggle then      
...
RunService.RenderStepped:Connect(function(step)     
	...
	RageTarget = nil      
	Spin = CLAMP(Spin + values.aatab.angles["spin speed"].Slider, 0, 360)      
	...
	if PlayerIsAlive then      
		...
		if values.aatab.fakelag["ping spike"].Toggle and values.aatab.fakelag["ping spike"].Active then      
			for count = 1, 20  do      
		...
		if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then      
			if values.rage.ragebot.enabled.Toggle then      
				local Origin = values.rage.ragebot.origin.Dropdown == "character" and LocalPlayer.Character.LowerTorso.Position + Vec3(0, 2.5, 0) or CamCFrame.p      
				...
				for _,Player in pairs(Players:GetPlayers()) do
					...
					spawn(function()
						if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and Player ~= LocalPlayer then      
							if TBLFIND(values.rage.ragebot.resolver.Jumbobox, "pitch") then      
							...
							if TBLFIND(values.rage.ragebot.resolver.Jumbobox, "roll") then      
								Player.Character.Humanoid.MaxSlopeAngle = 0      
							end  
							if TBLFIND(values.rage.ragebot.resolver.Jumbobox, "animation") then     
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and not Client.DISABLED and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and not Player.Character:FindFirstChildOfClass("ForceField") and GetDeg(CamCFrame, Player.Character.Head.Position) <= Stats["max fov"].Slider and Player ~= LocalPlayer then      
						if Player.Team ~= LocalPlayer.Team or values.rage.ragebot.teammates.Toggle and Player:FindFirstChild("Status") and Player.Status.Team.Value ~= LocalPlayer.Status.Team.Value and Player.Status.Alive.Value then      
							if Client.gun:FindFirstChild("Melee") and values.rage.ragebot["knifebot"].Toggle then      
		...
		if values.aatab.angles.enabled.Toggle and not DisableAA then      
			local Angle = -ATAN2(CamLook.Z, CamLook.X) + RAD(-90)      
			if values.aatab.angles["yaw base"].Dropdown == "spin" then      
				Angle = Angle + RAD(Spin)      
			end      
			if values.aatab.angles["yaw base"].Dropdown == "random" then      
				Angle = Angle + RAD(RANDOM(0, 360))      
			end      
			local Offset = RAD(-values.aatab.angles["yaw offset"].Slider - (values.aatab.angles.jitter.Toggle and Jitter and values.aatab.angles["jitter offset"].Slider or 0))      
			local CFramePos = CF(Root.Position) * CFAngles(0, Angle + Offset, 0)      
			if values.aatab.angles["yaw base"].Dropdown == "targets" then      
				local part      
				local closest = 9999      
				for _,plr in pairs(Players:GetPlayers()) do      
			...
			if values.aatab.angles["body roll"].Dropdown == "on" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "on" and RAD(values.aatab.angles["body roll offset"].Slider) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end  
            if values.aatab.angles["body roll"].Dropdown == "-4" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-4" and RAD(RANDOM(-4, 4)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-15" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-15" and RAD(RANDOM(-15, 15)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-30" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-30" and RAD(RANDOM(-30, 30)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-60" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-60" and RAD(RANDOM(-60, 60)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-180" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-180" and RAD(RANDOM(-180, 180)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-360" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-360" and RAD(RANDOM(-360, 360)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.aatab.angles["High poss"].Slider  
			end
			if values.aatab.angles["body roll"].Dropdown == "-374" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-374" and RAD(213) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  

      if values.aatab.angles["body roll"].Dropdown == "-262" then  
				Root.CFrame = Root.CFrame * CFAngles(values.aatab.angles["body roll"].Dropdown == "-262" and RAD(90) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  
			if values.aatab.angles["underground aa"].Toggle == true and underground == false then
                LocalPlayer.Character.LowerTorso.Root.C0 = LocalPlayer.Character.LowerTorso.Root.C0 * CFrame.Angles(0, 0, 0) * CFrame.new(0, values.aatab.angles["underground pos"].Slider, 0)
                underground = true
            end
			local Pitch = values.aatab.angles["pitch"].Dropdown == "none" and 0 or values.aatab.angles["pitch"].Dropdown == "up" and 1 or values.aatab.angles["pitch"].Dropdown == "down" and -1 or values.aatab.angles["pitch"].Dropdown == "dogshit" and -5/93 -13 or values.aatab.angles["pitch"].Dropdown == "funny" and 20 or values.aatab.angles["pitch"].Dropdown == "wind" and math.huge -5/0 -1 or values.aatab.angles["pitch"].Dropdown == "wind2" and 23 or values.aatab.angles["pitch"].Dropdown == "cool2" and -13 or values.aatab.angles["pitch"].Dropdown == "cool" and 12 or values.aatab.angles["pitch"].Dropdown == "back" and -92 or values.aatab.angles["pitch"].Dropdown == "God" and 15 or values.aatab.angles["pitch"].Dropdown == "fucker" and -13 or values.aatab.angles["pitch"].Dropdown == "random" and RANDOM(-100, 100)/10 or 2.5  
			if values.aatab.angles["extend pitch"].Toggle and (values.aatab.angles["pitch"].Dropdown == "up" or values.aatab.angles["pitch"].Dropdown == "down" or values.aatab.angles["pitch"].Dropdown == "dogshit") then  
				Pitch = (Pitch*2)/1.6  
			end  
		...
		if values.aatab.others["remove head"].Toggle then      
...
mt.__namecall = function(self, ...)      
	...
	if method == "FindPartOnRayWithIgnoreList" and args[2][1] == workspace.Debris then      
		if not checkcaller() or Filter then      
			...
			if RageTarget ~= nil then      
				local Origin = values.rage.ragebot.origin.Dropdown == "character" and LocalPlayer.Character.LowerTorso.Position + Vec3(0, 2.5, 0) or Camera.CFrame.p      
	...
	if method == "FireServer" and self.Name == "HitPart" then
		...
		if (values.rage.ragebot["prediction"].Toggle and RageTarget ~= nil) then      
			coroutine.wrap(function()      
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then      
					local hrp = RageTarget.Parent.HumanoidRootPart.Position      
					local oldHrp = RageTarget.Parent.HumanoidRootPart.OldPosition.Value      
...
mt.__newindex = function(self, i, v)      
	...
	elseif self:IsA("Humanoid") and i == "CameraOffset" then      
		if values.aatab.angles.enabled.Toggle and (values.aatab.angles["body roll"].Dropdown == "180" or values.aatab.angles["body roll"].Dropdown == "spin") and not DisableAA then      
			v = v + Vec3(0, -3.5, 0)      
		end      
	end      
